...
final class SMB2FileHandle: @unchecked Sendable {
    ...
    /// Queues non-overlapping positional writes and services the connection until all complete.
    /// The data remains owned by the caller for the duration of this method.
    func writeWindow(chunks: [(offset: UInt64, data: Data)]) throws {
        precondition(!chunks.isEmpty, "Empty write window.")
        ...
        try client.withThreadSafeContext { context in
            ...
            callbacks.withUnsafeMutableBufferPointer { callbackBuffer in
                for (index, chunk) in chunks.enumerated() {
                    ...
                    if result < 0 {
                        enqueueError = POSIXError(
                            Errno(rawValue: -result), description: client.errorString
                        )
                        break
                    }
                    submitted += 1
            ...
            if let enqueueError {
                if submitted > 0 {
                    try? client.serviceUntil(Array(callbacks.prefix(submitted)))
                }
                throw enqueueError
            }

            try client.serviceUntil(callbacks)
        }
    }
