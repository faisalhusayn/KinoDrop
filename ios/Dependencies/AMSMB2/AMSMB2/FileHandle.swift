...
final class SMB2FileHandle: @unchecked Sendable {
    ...
    /// Issues several non-overlapping positional writes in one batch and waits until every
    /// request has been acknowledged by the server. Unlike the single-write variant, this
    /// keeps multiple SMB2 WRITE requests in flight on the same connection (credit-based
    /// pipelining), which removes the stop-and-wait round-trip overhead between chunks.
    /// Buffers stay alive until all callbacks have fired.
    func writeWindow(chunks: [(offset: UInt64, data: Data)]) throws {
        precondition(
            !chunks.isEmpty, "Empty write window."
        )

        let handle = try handle.unwrap()
        try client.withThreadSafeContext { context in
            var cbs = [CBData](repeating: CBData(), count: chunks.count)
            for (index, chunk) in chunks.enumerated() {
                let result = chunk.data.withUnsafeBytes { buffer -> Int32 in
                    smb2_pwrite_async(
                        context, handle, buffer.baseAddress, .init(buffer.count), chunk.offset,
                        SMB2Client.generic_handler, &cbs[index]
                    )
                }
                if result < 0 {
