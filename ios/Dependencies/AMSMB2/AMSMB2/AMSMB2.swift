...
public class SMB2Manager: NSObject, NSSecureCoding, Codable, NSCopying, CustomReflectable, @unchecked Sendable {
    ...
    /**
     Uploads a local file with multiple non-overlapping SMB writes in flight.

     The window is bounded to avoid exhausting the server's SMB credits. This uses one
     connection and one remote handle; it is not SMB Multichannel.
     */
