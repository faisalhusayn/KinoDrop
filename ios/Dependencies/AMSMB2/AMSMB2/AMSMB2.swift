...
public class SMB2Manager: NSObject, NSSecureCoding, Codable, NSCopying, CustomReflectable, @unchecked Sendable {
    ...
    /**
     Uploads local file contents to a new location using pipelined writes. With reporting progress
     on about every window of written chunks.

     Instead of waiting for each SMB write to be acknowledged before issuing the next one, up to
     `pipelineSize` writes are kept in flight at non-overlapping offsets on the same connection.
     This removes the stop-and-wait round-trip overhead that limits single-write uploads.

     - Note: given url must be local file url otherwise it will throw error.
     - Note: If the remote file already exists, it will be overwritten.

     - Parameters:
       - at: url of a local file to be uploaded from.
       - toPath: path of new file to be uploaded to.
       - chunkSize: size of each write in bytes. Defaults to the server's negotiated maximum write size.
       - pipelineSize: number of writes kept in flight at the same time. Default value is 4.
       - progress: reports progress of written bytes count so far.
           User must return `true` if they want to continuing or `false` to abort uploading.
       - completionHandler: closure will be run after uploading is completed.
     */
    open func uploadItemPipelined(
        at url: URL, toPath: String, chunkSize: Int = 0, pipelineSize: Int = 4,
        progress: WriteProgressHandler, completionHandler: SimpleCompletionHandler
    ) {
        with(completionHandler: completionHandler) { client in
            guard try url.checkResourceIsReachable(), url.isFileURL,
                  let stream = InputStream(url: url)
            ...
            try self.writePipelined(
                client: client, from: stream, toPath: toPath,
                chunkSize: chunkSize, pipelineSize: pipelineSize, progress: progress
            )
        }
    }
    ...
    open func uploadItemPipelined(
        ...
        try await withCheckedThrowingContinuation { continuation in
            uploadItemPipelined(
                at: url, toPath: toPath, chunkSize: chunkSize, pipelineSize: pipelineSize,
                progress: progress, completionHandler: asyncHandler(continuation)
            )
        }
    }
...
extension SMB2Manager {
    ...
    private func writePipelined(
        client: SMB2Client, from stream: InputStream, toPath: String,
        chunkSize: Int, pipelineSize: Int, progress: WriteProgressHandler
    ) throws {
        let file = try SMB2FileHandle(forOverwritingIfExistsAtPath: toPath, on: client)
        let chunkSize = chunkSize > 0 ? chunkSize : file.optimizedWriteSize
        let pipelineSize = max(1, min(pipelineSize, 32))
        ...
        try stream.withOpenStream {
            ...
            while true {
                ...
                while window.count < pipelineSize {
                    ...
                    window.append((nextOffset, segment))
